...
const HistoricalDataGraphs = () => {
  ...
  const formatDateLabel = (dateStr, index, totalDays) => {
      ...
      <div className="bg-white rounded-lg shadow p-4">
          ...
          {data.map((day, index) => (
            <div key={index} className="flex-1 flex flex-col items-center">
              ...
              <div className="text-xs text-gray-500 mt-1">
                {formatDateLabel(day.date, index, data.length)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            ...
            <span className="ml-1 font-semibold">{data.reduce((sum, d) => sum + d.count, 0)}</span>
          </div>
          <div>
            <span className="text-gray-500">Peak:</span>
            <span className="ml-1 font-semibold">{Math.max(...data.map(d => d.count))}</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            ...
            <span className="ml-1 font-semibold">{(data.reduce((sum, d) => sum + d.count, 0) / data.length).toFixed(1)}</span>
          </div>
          <div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex items-end space-x-1 h-40 mb-4">
          {allDates.map((date, index) => (
            <div key={index} className="flex-1 flex flex-col items-center">
              <div className="w-full flex flex-col" style={{ height: '128px' }}>
                <div
                  className="w-full"
                  style={{
                    backgroundColor: '#dc2626',
                    height: `${(data.contamination[index].count / maxCount) * 100}%`,
                    minHeight: data.contamination[index].count > 0 ? '2px' : '0px'
                  }}
                ></div>
                <div
                  className="w-full"
                  style={{
                    ...
                    height: `${(data.boil_notice[index].count / maxCount) * 100}%`,
                    minHeight: data.boil_notice[index].count > 0 ? '2px' : '0px'
                  }}
                ></div>
                <div
                  className="w-full"
                  style={{
                    ...
                    height: `${(data.outage[index].count / maxCount) * 100}%`,
                    minHeight: data.outage[index].count > 0 ? '2px' : '0px'
                  }}
                ></div>
              </div>
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Alert Trends & Historical Data</h1>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100">Total Alerts</p>
              <p className="text-3xl font-bold">{calculateTotalAlerts(historicalData)}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold">{calculateAveragePerDay(historicalData)}</p>
            </div>
            <div className="text-4xl opacity-80">📈</div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100">Peak Day</p>
