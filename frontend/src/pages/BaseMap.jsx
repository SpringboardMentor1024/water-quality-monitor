...
// Logic to move the map and open the popup
const MapController = ({ activeStation }) => {
  const map = useMap();
  useEffect(() => {
    if (activeStation) {
      map.flyTo([activeStation.latitude, activeStation.longitude], 14, { duration: 1.5 });
    }
  }, [activeStation, map]);
  return null;
};
...
const BaseMap = ({ selectedStation }) => {
  ...
  const markerRefs = useRef({}); // Stores references to markers to trigger popups

  useEffect(() => {
    getStations().then(data => { if (data) setStations(data); });
  }, []);

  // Open popup when selected from index
  useEffect(() => {
          ...
          <Popup>
            <div className="p-2 min-w-[200px]">
              <div className="flex justify-between items-center mb-1">
                <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{stn.id}</span>
                <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
              </div>
              <h4 className="font-black text-blue-900 uppercase text-sm mb-0.5">{stn.name}</h4>
              <p className="text-[10px] text-gray-400 font-bold mb-3 italic">{stn.location}</p>
              ...
              <div className="grid grid-cols-2 gap-2 bg-blue-50/50 p-2 rounded-xl mb-4 border border-blue-100">
                <div className="text-center border-r border-blue-100">
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">pH Level</p>
                  <p className="text-xs font-black text-blue-700">7.2</p>
                </div>
                <div className="text-center">
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">DO (mg/L)</p>
